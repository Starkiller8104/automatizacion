...
def _series_maps_for_template(days):
    ...
    def _as_map_from_range(series_key, start, end):
        ...
        m = {}
        for o in obs:
            _f = parse_any_date(o.get('fecha')); _v = try_float(o.get('dato'))
            if _f and (_v is not None): m[_f.date().isoformat()] = _v
        return m
    start = days[0].isoformat(); end = days[-1].isoformat()
    m_fix  = _as_map_from_range('USD_FIX', start, end)
    m_eur  = _as_map_from_range('EUR_MXN', start, end)
    ...
    cet_end   = days[-1].isoformat()
    def _asof(key):
        m = _as_map_from_range(key, cet_start, cet_end)
        vals,_ = _ffill_asof_with_flags_from_map(m, header_dates); return vals
    c28  = _asof('CETES_28'); c91 = _asof('CETES_91'); c182 = _asof('CETES_182'); c364 = _asof('CETES_364')
    m_t28  = _as_map_from_range('TIIE_28',  start, end)
    m_t91  = _as_map_from_range('TIIE_91',  start, end)
    m_t182 = _as_map_from_range('TIIE_182', start, end)
    _obs_obj = sie_range('SF61745', start, end); m_obj = {}
    for o in _obs_obj:
        _f = parse_any_date(o.get('fecha')); _v = try_float(o.get('dato'))
        if _f and (_v is not None): m_obj[_f.date().isoformat()] = _v
    t28,_  = _ffill_with_flags(m_t28,  header_dates)
    t91,_  = _ffill_with_flags(m_t91,  header_dates)
    t182,_ = _ffill_with_flags(m_t182, header_dates)
    tobj,_ = _ffill_with_flags(m_obj,   header_dates)
...
def export_indicadores_template_bytes(template_path: str = _TEMPLATE_PATH):
    from app_writer_layout_v3 import write_layout_v3
    days = _build_header_dates_6b(); S = _series_maps_for_template(days)
    try: mv = rolling_movex_for_last6(window=movex_win)
    except Exception: mv = None
    try: mpct = float(margen_pct)
    except Exception: mpct = 0.20
    payload = {
        "DOLAR": {"5":{"F": (S['fix'][-2] if len(S['fix'])>=2 else None), "G": (S['fix'][-1] if S['fix'] else None)}},
        "YEN":   {"10":{"F": (S['jpy'][-2] if len(S['jpy'])>=2 else None), "G": (S['jpy'][-1] if S['jpy'] else None)},
                  "11":{"G": ((S['fix'][-1]/S['jpy'][-1]) if (S['fix'] and S['jpy'] and S['fix'][-1] and S['jpy'][-1]) else None)}},
        "EURO":  {"14":{"F": (S['eur'][-2] if len(S['eur'])>=2 else None), "G": (S['eur'][-1] if S['eur'] else None)},
                  "15":{"G": ((S['eur'][-1]/S['fix'][-1]) if (S['eur'] and S['fix'] and S['eur'][-1] and S['fix'][-1]) else None)}},
        "UDIS":  {"18":{"F": (S['udis'][-2] if len(S['udis'])>=2 else None), "G": (S['udis'][-1] if S['udis'] else None)}},
        ...
        "CETES": {"27":{"G": ((S['c28'][-1] /100.0) if S['c28'] and S['c28'][-1] is not None else None)},
                  "28":{"G": ((S['c91'][-1] /100.0) if S['c91'] and S['c91'][-1] is not None)},
                  "29":{"G": ((S['c182'][-1]/100.0) if S['c182'] and S['c182'][-1] is not None)},
                  "30":{"G": ((S['c364'][-1]/100.0) if S['c364'] and S['c364'][-1] is not None)}},
        "UMA":   {"33":{"G":S['uma'].get('diario')}, "34":{"G":S['uma'].get('mensual')}, "35":{"G":S['uma'].get('anual')}},
    }
    if mv and isinstance(mv, (list, tuple)) and len(mv) >= 2:
        compra = [(x*(1 - mpct/100) if x is not None else None) for x in mv]
