...
# ======================
# Diagnóstico Banxico / Conectividad
# ======================
import streamlit as st
import requests
from datetime import timedelta

def _probe_banxico(series_id="SF43718", days=10):
    """Devuelve dict con estado de prueba de conectividad y ejemplo de datos."""
    info = {"ok": False, "status": None, "count": 0, "last_date": None, "error": None, "token_present": bool(BANXICO_TOKEN)}
    try:
        end = _cdmx_now().date()
        ...
        obs = data[0].get("datos",[]) if data else []
        info["count"] = len(obs)
        if obs:
            info["ok"] = True
            info["last_date"] = obs[-1].get("fecha")
    except Exception as e:
        info["error"] = str(e)
    return info

with st.expander("🔎 Diagnóstico Banxico / Conectividad", expanded=False):
    pi = _probe_banxico("SF43718", days=14)
    st.write({
        "token_present": pi["token_present"],
        "status_code": pi["status"],
        "obs_count": pi["count"],
        "last_date": pi["last_date"],
        "error": pi["error"],
    })
    if not pi["ok"]:
        if not pi["token_present"]:
            st.error("No se detectó **BANXICO_TOKEN**. Configúralo en *Secrets* o variable de entorno.")
        if pi["status"] in (401, 403):
            st.error("Banxico SIE devolvió **{}**: revisa el token (vigencia/valor).".format(pi["status"]))
        elif pi["status"] == 429:
            st.warning("**429 Too Many Requests**: Banxico está limitando. Reintenta en unos minutos.")
        elif pi["status"] is None:
            st.error("No se recibió respuesta: revisa salida a Internet/Firewall/Proxy.")
        else:
            st.warning("Respuesta SIE inesperada. Detalle en 'error'.")

# Hook para bloquear exportación si no hay datos reales
def _has_real_values(vals: dict) -> bool:
    keys = ["fix","eur","jpy","udis","tobj","t28","t91","t182","c28","c91","c182","c364"]
    for k in keys:
        v = vals.get(k, (None,None))
        if any(x not in (None, "") for x in v):
            return True
    return False

# Reemplaza el botón de descarga para validar que existan datos
try:
    # buscamos donde se arma vals y el botón; aquí reexponemos un botón seguro
